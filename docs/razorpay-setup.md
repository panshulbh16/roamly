...
In Razorpay → Webhooks, add `https://roamly-trip-planner.pb116.chatgpt.site/api/billing/webhook` for the `order.paid` event with the webhook secret above. Checkout's success callback grants the pass immediately after verifying the payment signature; the webhook is the backstop for buyers who close the tab. Both are idempotent: only the first call that flips an order from `created` to `paid` grants 30 days.
