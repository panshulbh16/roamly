...
function ResolvedCostBreakdown({ input, initialBand, embedded }: { input: CostInput; initialBand?: CostBand; embedded: boolean }) {
  ...
  const [journey, setJourney] = useState<Journey>({ rooms: Math.ceil(input.travelers / 2), mode: "Not included", fare: 0 });
  const estimate = estimateCost(input, band, customRates, journey);
  ...
  const labels: Record<string, string> = { stay: "Accommodation", food: "Food & drinks", transport: "Local transport", activities: "Activities", journey: `${journey.mode} · return journey`, buffer: "10% contingency" };
  return <section className={embedded ? "cost-page trip-cost-result" : "workspace cost-page"} aria-label="Trip cost estimate">
    ...
    <div className="cost-layout">
      <section className="panel cost-total"><span className="eyebrow">{input.budget.toUpperCase()} · WHOLE GROUP</span><h2 aria-live="polite">{range(estimate.low, estimate.high)}</h2><p>{journey.mode === "Not included" ? "Estimated on-the-ground spending" : "Estimated trip spending including your return fare"}</p>{currency !== active.currency && <p className="form-note">Showing {active.currency}{!error ? ` while converting to ${currency}…` : `; ${currency} conversion unavailable.`}</p>}<p className="subtext">{`${range(estimate.low / input.travelers, estimate.high / input.travelers)} per person`}</p>
      ...
      <p className="form-note">Planning allowances, not booking quotes. {journey.mode === "Not included" ? "Flights, intercity travel are excluded until you add a fare below. " : "Your entered return fare is included. "}Visas, insurance, shopping and unentered booking taxes/fees are excluded.</p></section>
