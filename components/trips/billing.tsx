...
export function BillingControls({ price = "₹499" }: { price?: string }) {
  ...
  return <div>
    ...
    {!status?.plus&&<p className="form-note">{price} billed monthly through Razorpay. Renews monthly until cancelled, for up to 10 years. Includes 20 AI plans per day; replacing one day uses one plan.</p>}
    {!subscribed&&<button className="primary" disabled={busy||!status} onClick={async()=>{setBusy(true);setMessage("");try{const result=await request("checkout","POST");window.location.assign(result.url);}catch(e){setMessage((e as Error).message);setBusy(false);}}}>Continue to Razorpay · {price}/month</button>}
