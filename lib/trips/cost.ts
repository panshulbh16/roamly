...
export function estimateCost(input: CostInput, band: CostBand) {
  ...
  const rows = (Object.keys(rates) as (keyof typeof rates)[]).map(key => ({ key, low: Math.round(rates[key] * factor * quantities[key] * 0.8), high: Math.round(rates[key] * factor * quantities[key] * 1.3) }));
...
export function validRate(data: unknown, currency: string): data is { rate: number; date: string } {
  if (!data || typeof data !== "object") return false;
  const d = data as Record<string, unknown>;
  return d.base === "USD" && d.quote === currency && typeof d.rate === "number" && Number.isFinite(d.rate) && d.rate > 0 && d.rate < 1e9 && typeof d.date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(d.date);
}
